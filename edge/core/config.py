...
DUPLICATE_IOU_THRESHOLD = min(
    1.0,
    max(0.0, env_float_alias(("DUPLICATE_IOU_THRESHOLD",), 0.85)),
)
...
TRACK_STITCH_ENABLED = env_bool_alias(("TRACK_STITCH_ENABLED",), True)
TRACK_STITCH_RECENT_SECONDS = max(0.0, env_float_alias(("TRACK_STITCH_RECENT_SECONDS",), 5.0))
TRACK_STITCH_MAX_DISTANCE = max(0.0, env_float_alias(("TRACK_STITCH_MAX_DISTANCE",), 160.0))
TRACK_STITCH_MIN_IOU = min(1.0, max(0.0, env_float_alias(("TRACK_STITCH_MIN_IOU",), 0.04)))
...
TRACK_EXIT_WITHOUT_ENTRY_MIN_FRAMES = max(
    1,
    env_int_alias(("TRACK_EXIT_WITHOUT_ENTRY_MIN_FRAMES",), 12),
)
TRACK_EVENT_COOLDOWN_SECONDS = max(
    0.0,
    env_float_alias(("TRACK_EVENT_COOLDOWN_SECONDS", "EVENT_COOLDOWN_SECONDS"), 12.0),
)
TRACK_SAME_TRACK_OUT_COOLDOWN_SECONDS = max(
    0.0,
    env_float_alias(("TRACK_SAME_TRACK_OUT_COOLDOWN_SECONDS",), 8.0),
)
TRACK_REENTRY_COOLDOWN_SECONDS = max(
    TRACK_EVENT_COOLDOWN_SECONDS,
    env_float_alias(("TRACK_REENTRY_COOLDOWN_SECONDS",), 90.0),
)
TRACK_ROI_POINT = env_alias(("TRACK_ROI_POINT", "COUNTING_POINT"), "feet").strip().lower() or "feet"
