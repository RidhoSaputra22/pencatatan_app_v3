...
CONFIG_GROUPS: List[Dict[str, str]] = [
    ...
    },
    {
        "id": "recording",
        "label": "Rekaman CCTV",
        "description": "Mode simpan dan segmentasi backup rekaman otomatis.",
    },
    {
        "id": "yolo",
    ...
    },
    {
        "id": "tracking",
        "label": "Tracking",
        "description": "Metode tracker, entry/exit gate, dan cooldown event.",
    },
    {
        "id": "reid",
        "label": "ReID Identity",
...
CONFIG_ITEMS: List[Dict[str, Any]] = [
    ...
    {
        "key": "EDGE_LOCAL_FILE_STOP_ON_END",
        "group": "stream",
        "label": "Berhenti saat file habis",
        "type": "bool",
        "default": "false",
        "help": "Hanya berlaku untuk file video lokal. Jika aktif, playback tidak diulang dan edge worker berhenti memproses saat file mencapai akhir.",
    },
    {
    ...
    {
        "key": "EDGE_RECORDING_SAVE_MODE",
        "group": "recording",
        "label": "Mode simpan",
        "type": "select",
        "default": "detection",
        "options": [_option("detection", "Deteksi / overlay"), _option("raw", "Raw tanpa overlay")],
    },
    {
        "key": "EDGE_RECORDING_FILE_MODE",
        "group": "recording",
        "label": "Mode record",
        "type": "select",
        "default": "segment",
        "options": [
            _option("segment", "Segmen per durasi"),
            _option("session", "Satu file per sesi edge"),
        ],
        "help": "Segmen memotong file otomatis per durasi. Sesi edge menyimpan satu file penuh sejak recorder edge mulai sampai worker berhenti, sumber berubah, atau gap stream memicu reset.",
    },
    {
        "key": "EDGE_RECORDING_SEGMENT_MINUTES",
        "group": "recording",
        "label": "Durasi segmen",
        "type": "int",
        "default": "10",
    ...
    },
    {
        ...
        "max": 120,
        "unit": "fps",
        "help": "0 berarti mengikuti FPS proses/stream.",
    },
    {
        "key": "EDGE_RECORDING_MAX_GAP_SECONDS",
        "group": "recording",
        "label": "Reset saat gap stream",
        "type": "float",
        "default": "5",
        "min": 1,
        "max": 3600,
    ...
    {
        "key": "TRACKER_METHOD",
        "aliases": ["TRACKING_METHOD", "TRACKER_BACKEND"],
        "group": "tracking",
        "label": "Metode tracker",
        "type": "select",
        "default": "auto",
        "options": [
            _option("auto", "Auto (DeepSORT bila tersedia)"),
            _option("bytetrack", "ByteTrack"),
            _option("deepsort", "DeepSORT + ReID"),
            _option("centroid", "Centroid"),
        ],
        "help": "Pilih algoritma tracking utama. ByteTrack dan Centroid lebih ringan; jika IDENTITY_MODE = reid, worker akan mengambil body embedding terpisah dari crop bbox.",
    },
    {
        "key": "FORCE_CENTROID",
        "group": "tracking",
        "label": "Paksa centroid saat auto",
        "type": "bool",
        "default": "false",
        "help": "Opsi legacy: hanya berlaku saat metode tracker Auto.",
    },
    {
        "key": "TRACK_MAX_AGE",
    ...
    {
        "key": "BYTETRACK_HIGH_THRESH",
        "group": "tracking",
        "label": "High threshold ByteTrack",
        "type": "float",
        "default": "0.45",
        "min": 0,
        "max": 1,
    ...
    {
        "key": "BYTETRACK_LOW_THRESH",
        "group": "tracking",
        "label": "Low threshold ByteTrack",
        "type": "float",
        "default": "0.1",
        "min": 0,
        "max": 1,
    ...
    {
        "key": "BYTETRACK_MATCH_THRESH",
        "group": "tracking",
        "label": "Match threshold ByteTrack",
        "type": "float",
        "default": "0.8",
        "min": 0,
        "max": 1,
    ...
    {
        "key": "BYTETRACK_NEW_TRACK_THRESH",
        "group": "tracking",
        "label": "New track threshold ByteTrack",
        "type": "float",
        "default": "0.45",
        "min": 0,
        "max": 1,
    ...
    {
        "key": "BYTETRACK_MIN_BOX_AREA",
        "group": "tracking",
        "label": "Min area box ByteTrack",
        "type": "float",
        "default": "10",
        "min": 0,
        "max": 1000000,
    ...
    {
        "key": "TRACK_STITCH_ENABLED",
        "group": "tracking",
        "label": "Sambung track putus",
        "type": "bool",
        "default": "true",
        "help": "Cocokkan track baru dengan track lama yang baru hilang agar stream jitter tidak dihitung sebagai pengunjung baru.",
    },
    {
        "key": "TRACK_STITCH_RECENT_SECONDS",
        "group": "tracking",
        "label": "Jendela sambung track",
        "type": "float",
        "default": "5",
        "min": 0,
        "max": 30,
    ...
    {
        "key": "TRACK_STITCH_MAX_DISTANCE",
        "group": "tracking",
        "label": "Jarak sambung track",
        "type": "float",
        "default": "160",
        "min": 0,
        "max": 1000,
    ...
    {
        "key": "TRACK_STITCH_MIN_IOU",
        "group": "tracking",
        "label": "IoU sambung track",
        "type": "float",
        "default": "0.04",
        "min": 0,
        "max": 1,
    ...
    },
    {
        ...
        "type": "select",
        "default": "reid",
        "options": [_option("reid", "Body ReID"), _option("face", "Face embedding"), _option("track", "Track ID fallback")],
    },
    {
    ...
    {
        "key": "REID_FRAME_INTERVAL",
        "group": "reid",
        "label": "Interval ekstraksi ReID",
        "type": "int",
        "default": "3",
        "min": 1,
        "max": 60,
    ...
    {
        "key": "REID_MIN_CROP_WIDTH",
        "group": "reid",
        "label": "Lebar crop minimum ReID",
        "type": "int",
        "default": "32",
        "min": 8,
        "max": 2000,
    ...
    {
        "key": "REID_MIN_CROP_HEIGHT",
        "group": "reid",
        "label": "Tinggi crop minimum ReID",
        "type": "int",
        "default": "64",
        "min": 8,
        "max": 2000,
    ...
    {
        "key": "REID_CROP_PADDING_RATIO",
        "group": "reid",
        "label": "Padding crop ReID",
        "type": "float",
        "default": "0.08",
        "min": 0,
        "max": 0.5,
    ...
    {
        "key": "REID_STITCH_MATCH_THRESHOLD",
        "group": "reid",
        "label": "Threshold stitch ReID",
        "type": "float",
        "default": "0.82",
        "min": 0,
        "max": 1,
...
CONFIG_HINTS: Dict[str, str] = {
    ...
    "EDGE_LOCAL_FILE_STOP_ON_END": "Jika aktif, file video lokal berhenti total saat mencapai frame terakhir. Edge worker tidak mengulang file dari awal sampai sumber diganti atau opsi ini dimatikan.",
    ...
    "EDGE_RECORDING_ENABLED": "Mengaktifkan atau mematikan backup otomatis ke daftar Rekaman CCTV. Saat dimatikan, file baru tidak dibuat sampai diaktifkan lagi. Rekaman parsial mode segmen dibuang, sedangkan mode sesi penuh menyimpan sesi yang sudah berjalan saat worker menutup recorder.",
    "EDGE_RECORDING_SAVE_MODE": "Menentukan isi file backup otomatis. Deteksi menyimpan overlay bbox/ROI/info seperti preview. Raw menyimpan gambar bersih sebelum overlay, tetapi deteksi dan counting tetap berjalan.",
    "EDGE_RECORDING_FILE_MODE": "Mengatur apakah rekaman otomatis dipotong per durasi tetap atau disimpan sebagai satu file penuh per sesi edge. Mode sesi penuh membuat file baru muncul setelah sesi berakhir.",
    "EDGE_RECORDING_SEGMENT_MINUTES": "Durasi setiap file rekaman otomatis saat mode record = segmen. Perubahan berlaku pada segmen berikutnya setelah edge worker mengambil config. Nilai lebih pendek membuat file cepat muncul tetapi jumlah file bertambah.",
    "EDGE_RECORDING_FPS": "FPS file rekaman otomatis. 0 mengikuti FPS proses/stream. Nilai tinggi membuat video lebih halus tetapi ukuran file dan beban encoder naik.",
    "EDGE_RECORDING_MAX_GAP_SECONDS": "Jika frame berhenti lebih lama dari nilai ini, recorder memulai sesi/segmen baru agar file hasil tidak berisi loncatan waktu besar.",
    ...
    "TRACKER_METHOD": "Metode tracking utama. Auto mempertahankan perilaku lama: DeepSORT bila tersedia, atau centroid bila dipaksa/DeepSORT tidak tersedia. ByteTrack ringan dan menjaga ID lebih baik dari centroid pada deteksi yang putus-nyambung. Saat IDENTITY_MODE = reid, worker dapat mengambil body embedding terpisah tanpa harus memakai DeepSORT.",
    "FORCE_CENTROID": "Opsi legacy untuk Auto: memaksa tracker memakai centroid sederhana. Jika Metode tracker dipilih eksplisit ke ByteTrack, DeepSORT, atau Centroid, pilihan metode tersebut yang dipakai.",
    ...
    "BYTETRACK_HIGH_THRESH": "Confidence minimum untuk asosiasi utama ByteTrack dan kandidat track baru. Terlalu tinggi membuat orang redup/jauh mudah putus. Terlalu rendah membuat noise lebih mudah dipakai sebagai track.",
    "BYTETRACK_LOW_THRESH": "Confidence minimum untuk asosiasi kedua ByteTrack. Nilai ini juga menurunkan threshold detector saat ByteTrack aktif agar bbox low-confidence tersedia. Terlalu rendah menambah noise dan beban matching.",
    "BYTETRACK_MATCH_THRESH": "Batas kelonggaran matching ByteTrack; nilai lebih tinggi lebih longgar menyambung track, nilai lebih rendah lebih ketat. Terlalu longgar bisa menukar ID saat orang berdekatan.",
    "BYTETRACK_NEW_TRACK_THRESH": "Confidence minimum untuk membuat track ByteTrack baru. Biasanya sama atau sedikit di atas high threshold. Terlalu rendah membuat false track, terlalu tinggi menunda track valid.",
    "BYTETRACK_MIN_BOX_AREA": "Area bbox minimum untuk ByteTrack. Naikkan jika banyak deteksi kecil/noise, turunkan jika kamera menangkap orang dari jauh.",
    "TRACK_STITCH_ENABLED": "Mengaktifkan anti double-count saat tracker membuat ID baru karena frame relay loncat atau deteksi hilang sebentar. Track baru yang muncul dekat track lama akan memakai visitor key lama.",
    "TRACK_STITCH_RECENT_SECONDS": "Berapa lama track lama disimpan untuk dicocokkan ulang. Terlalu tinggi bisa menggabungkan orang berbeda, terlalu rendah kurang membantu saat stream putus agak lama.",
    "TRACK_STITCH_MAX_DISTANCE": "Jarak maksimum pusat bbox untuk menyambung track lama dan baru. Naikkan jika frame relay sering loncat jauh, turunkan jika orang di scene sangat berdekatan.",
    "TRACK_STITCH_MIN_IOU": "IoU minimum untuk menyambung track walau jarak pusat agak berubah. Nilai kecil membantu saat bbox berubah bentuk, tetapi terlalu kecil meningkatkan risiko salah gabung.",
    ...
    "IDENTITY_MODE": "Sumber identitas visitor unik. ReID memakai body embedding yang bisa diambil terpisah dari tracker, face memakai embedding wajah, track memakai ID tracker fallback. Mode yang tidak sesuai data kamera bisa membuat unique visitor pecah atau tergabung salah.",
    ...
    "REID_FRAME_INTERVAL": "Berapa sering worker mengekstrak body embedding baru dari crop orang. Nilai kecil lebih responsif, tetapi beban CPU/GPU naik. Nilai besar lebih hemat, tetapi identitas baru lebih lambat terkunci.",
    "REID_MIN_CROP_WIDTH": "Lebar minimum bbox sebelum body embedding diambil. Naikkan jika banyak crop kecil/noisy dari orang jauh. Terlalu tinggi membuat orang kecil tidak ikut ReID dan lebih sering fallback ke track.",
    "REID_MIN_CROP_HEIGHT": "Tinggi minimum bbox sebelum body embedding diambil. Naikkan jika banyak crop sempit atau parsial. Terlalu tinggi membuat pengunjung jauh atau hanya tampak separuh tubuh sulit mendapat identity yang stabil.",
    "REID_CROP_PADDING_RATIO": "Rasio padding di sekitar bbox saat membuat crop body ReID. Sedikit padding membantu menangkap konteks pakaian, tetapi terlalu besar memasukkan background dan menurunkan akurasi.",
    "REID_STITCH_MATCH_THRESHOLD": "Kemiripan minimum embedding untuk menyambung track centroid/ByteTrack yang baru putus ke visitor lama. Terlalu tinggi membuat stitching gagal, terlalu rendah berisiko menggabungkan orang berbeda.",
